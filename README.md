...
- Vms on VPC (Virtual Private Cloud): In this case th ip provided on UI or added manually on json and manifests can be the private ip of the VPC where virtual machines run. In this case if it is not possible reach the vms on public internet, run this project on a local vm inside the VPC using the internal ip of the vms (inside the VPC). Keep in mind that all vms have to connect on internet to configure the packages repositories. 
