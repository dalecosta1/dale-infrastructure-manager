...
- Vms on VPC (Virtual Private Cloud): In this case th ip provided on UI or added manually on json and manifests can be the private ip of the VPC where virtual machines run. In this case if it is not possible reach the vms on public internet, run this project on a local VPC vm using the internal iè (private ip inside the VPC). Keep in mind that all vms have to connect on internet to configure the packages repositories. 
...
If you work on on-prem network, you can use the internal nodes ip of the cluster (eg: 192.168.1.214, 192.168.1.101).
The dale-infrastructure-manager fit well for both cases on-prem & cloud 🛸🛸
